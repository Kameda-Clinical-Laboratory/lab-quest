...
import { LoginStampPopup } from '@/components/LoginStampPopup'
...
export function HomeMap() {
  const { currentStudent, todayQueue, mockToday, setMockToday, stages, loginStampResult } =
    useAppState()
  ...
  return (
    <div className="space-y-5">
      {loginStampResult?.isNew && <LoginStampPopup dayNumber={loginStampResult.dayNumber} />}
        ...
        <div className="space-y-5">
          {(todayQueue.isFinalVisit || p.clearedStageIds.length > 0) && (
            <Card className="quest-frame border-0 bg-transparent">
              <CardHeader>
                <CardTitle className="text-lg text-emerald-50">{'\u30dc\u30b9\u6226'}</CardTitle>
              </CardHeader>
              <CardContent>
